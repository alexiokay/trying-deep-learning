...
train_gen_transformed = train_datagen.flow(X_train_scaled, y_train, batch_size=32, shuffle=True)
train_gen_original = tf.data.Dataset.from_tensor_slices((X_train_scaled, y_train))
train_gen_original = train_gen_original.shuffle(buffer_size=len(X_train_scaled))
train_gen_original = train_gen_original.batch(batch_size=32)

train_gen = tf.data.experimental.sample_from_datasets([train_gen_transformed, train_gen_original])
...
# Add more dense layers with decreasing number of neurons
...
early_stopping_loss = EarlyStopping(monitor='loss', patience=4)
early_stopping_acc = EarlyStopping(monitor='val_accuracy', patience=4)


hist = model.fit(train_gen, epochs=30, validation_data=(X_valid, y_valid), callbacks=[tensorboard_callback, early_stopping_loss, early_stopping_acc])
